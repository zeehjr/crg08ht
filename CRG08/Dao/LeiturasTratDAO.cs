...
namespace CRG08.Dao
{
    public class LeiturasTratDAO
    {
        public static bool inserirLeiturasTratamento(List<LeiturasTrat> leituras,int pIndiceDaLeitura, Ciclos ciclo)
        {
                ...
                using (FbCommand cmd = new FbCommand())
                {
                    try
                    {
                        ...
                        for (int i = 0; i < leituras.Count; i++)
                        {
                            ...
                            {
                                var l = leituras[j];
                                /*meioComando = "INSERT INTO LEITURAS_TRATAMENTO(HORARIO,T1,T2,T3,T4,CICLO) VALUES('" +
                                              leituras[j].horario + "'," + 
                    ...
                    catch (FbException fbError)
                    {
                        ...
                        logErro.descricao = "Erro no inserir as Leituras do Tratamento";
        ...
        public static bool inserirLeiturasTratamento150(List<LeiturasTrat> leituras, int pIndiceDaLeitura, Ciclos ciclo)
        {
            using (FbConnection fbConn = new FbConnection(Util.DAO.Conn))
            {
                using (FbCommand cmd = new FbCommand())
        ...
        public static bool DeletaLeiturasTratamento(int id, int crg)
        {
            using (FbConnection fbConn = new FbConnection(Util.DAO.Conn))
            {
                using (FbCommand cmd = new FbCommand())
                {
                    try
                    {
                        fbConn.Open();
                        cmd.Connection = fbConn;
                        cmd.CommandText = "DELETE FROM LEITURAS_TRATAMENTO WHERE CICLO = " + id ;
                        cmd.ExecuteNonQuery();
                        return true;
                    }
                    catch (FbException fbError)
        ...
        public static List<LeiturasTrat> ListaLeiturasTratamento(Ciclos ciclo)
        {
            using (FbConnection fbConn = new FbConnection(Util.DAO.Conn))
            {
                using (FbCommand cmd = new FbCommand())
                {
                    try
                    {
                        fbConn.Open();
                        cmd.Connection = fbConn;
                        cmd.CommandText = "SELECT * FROM LEITURAS_TRATAMENTO WHERE CICLO = " + ciclo.id + " ORDER BY ID";
                        FbDataReader dr = cmd.ExecuteReader();
                        List<LeiturasTrat> listaLeituras = new List<LeiturasTrat>();
                        while (dr.Read())
                        {
                            LeiturasTrat leituras = new LeiturasTrat();
                            leituras.idLeiturasTrat = (int)dr["ID"];
                    ...
                    catch (FbException fbError)
                    {
                        LogErro logErro = new LogErro();
                        logErro.crg = ciclo.crg;
                        logErro.descricao = "Erro ao listar as Leituras do Tratamento";
                        logErro.data = DateTime.Now;
                        logErro.maisDetalhes = fbError.Message;
                        LogErroDAO.inserirLogErro(logErro, 0);
                        return null;
            ...
            using (FbConnection fbConn = new FbConnection(Util.DAO.Conn))
            {
                using (FbCommand cmd = new FbCommand())
                {
                    try
                    {
                        fbConn.Open();
                        cmd.Connection = fbConn;
                        cmd.CommandText = "SELECT COUNT(ID) AS ID FROM LEITURAS_TRATAMENTO WHERE CICLO = " + id;
                        FbDataReader dr = cmd.ExecuteReader();
                    ...
                    catch (FbException fbError)
                    {
                        LogErro logErro = new LogErro();
                        logErro.crg = crg;
                        logErro.descricao = "Erro ao retornar quantidade de leituras do tratamento";
                        logErro.data = DateTime.Now;
                        logErro.maisDetalhes = fbError.Message;
                        LogErroDAO.inserirLogErro(logErro, 0);
                        return -1;
